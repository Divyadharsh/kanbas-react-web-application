...
export default function PeopleDetails({ fetchUsers }: { fetchUsers: () => void; }) {
  ...
  return (
    <div className="position-fixed top-0 end-0 bottom-0 bg-white p-4 shadow w-25">
      ...
      <div className="container text-danger fs-4">
        {!editing && (
      ...
      <button onClick={() => navigate(`/Kanbas/Courses/${cid}/People`)} className="btn btn-secondary float-start float-end me-2" > Cancel </button>
    </div>
  );
}